...
product_data_class = product_data[(product_data['reviews.rating'] == 3) | (product_data['reviews.rating'] == 5) | (product_data['reviews.rating'] == 4) | (product_data['reviews.rating'] == 2) | (product_data['reviews.rating'] == 1)]
...
#y_test_sorted=y_test.sort_index(0,ascending=True)
#print(y_test_sorted)
#print(y_test['reviews.rating'])
...
for index,row in copy_product_data.iterrows():
    if (pd.isnull(row['reviews.rating'])):
        ...
        rating_prediction = loaded_model.predict(text_of_null_transformed)[0]
...
scores = cross_val_score(loaded_model,X,Y, cv=10)
